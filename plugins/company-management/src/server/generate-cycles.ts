...
export type CycleType = 'Năm' | 'Quý' | 'Tháng' | 'Tuần';

export interface CycleRecordInput {
  /** Local key used to resolve `parent_cycle` after each record is inserted; never persisted. */
  key: string;
...
function isoWeekRange(year: number, weekNo: number): { start: Date; end: Date } {
 ...
 * `year`.
 */
export function buildCycleRecordsForYear(year: number): CycleRecordInput[] {
  const records: CycleRecordInput[] = [];
